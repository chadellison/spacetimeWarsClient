...
class Layout extends React.Component {
  ...
  componentDidMount() {
    this.props.syncClocksAction(REQUEST_COUNT);
  ...
  render = () => {
    ...
    const {name, index } = this.props.user;
    return (
      <div className="layout" onKeyDown={this.handleKeyDown}>
        <div className='game row'>
          ...
          />
          {user.index >= 0 && !modal.display && <GameButton
            className={'gameButton'}
            onClick={() => this.props.updateModalAction({...modal, display: 'selection'})}
            buttonText={'shop'}
          />}
          <HeaderButtons />
          // <HeaderButtons updateState={this.updateState} />
          {user.name && <PlayerData
            // modal={modal}
            // defenseData={defenseData}
            // activePlayer={activePlayer}
            // updateState={this.updateState}
